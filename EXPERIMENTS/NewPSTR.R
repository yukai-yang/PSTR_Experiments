...
WbLinTest <- function(aDat, ik=2, im=1, iq=NULL, iB=100)
# linearity test
# input: aDat, an array of the data, T which N
# ik, NOT the lag, number of x
# im, the biggest number of switches, or better say, the biggest order of the test
{
  ...
  for(mter in 1:im){
    ...
    for(bter in 1:iB){
      ...
      qLM = c(qLM, sLMTEST(iT=iT,iN=iN,vU=vu,mX=mX,mW=mW,mM=mM,s2=ss,mX2=mX2,invXX=invXX))
    }

    RET = c(RET, LM<=quantile(qLM,prob=0.95))
  }
...
WbLinTest2 <- function(aDat, ik=2, im=1, iq=NULL, iB=100)
  # linearity test
  # input: aDat, an array of the data, T which N
  # ik, NOT the lag, number of x
  # im, the biggest number of switches, or better say, the biggest order of the test
