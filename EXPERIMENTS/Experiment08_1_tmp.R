## Experiment08_1_tmp.R
## vc = C2, misspecified m equivalent to B2=-B1
## Power of test of no remaining heterogeneity, table 8
## author: Yukai Yang
...
load("TMP/EXPERIMENT08_1.RData")

## experiment

cat("Experiment 08_1_tmp starts!\n")
ptm <- proc.time()
...
for(tter in 3:length(vT)) for(nter in 4:length(vN)){
  ...
  for(mter in 1:iM){
    ...
    EST1 = try( EstPSTR(aDat=mX1,im=1,par=c(log(GA),vc[1])), T)
    ...
    EST2 = try( EstPSTR(aDat=mX2,im=1,par=c(log(GA),vc[1])), T)
...
save.image("RESULTS/EXPERIMENT08_1.RData")
