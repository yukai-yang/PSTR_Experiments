...
cat("Experiment EXPWARP04_1 starts!\n")
ptm <- proc.time()
...
for(tter in 1:length(vT)) for(nter in 1:length(vN)){
  ...
  sfExport(list=c("fTF","WCBTVTest","sLMTEST","EstPSTR","DerGFunc","iT","iN","GA","vc","msigma","B01","B1","ikk","Theta","kappa","chS"))
...
save.image("RESULTS/EXPWARP04_1.RData")
