## Empirical critical values, EXPECV02_1.R #new
## Power of homogeneity test, table 2#new
## vc = C1
## author: Yukai Yang
...
aRet1 = array(0, dim=c(3, 3,length(vT),length(vN), iM)) #hom
aRet2 = array(0, dim=c(3, 3,length(vT),length(vN), iM)) #het 

cat("Experiment EXPECV02_1 starts!\n")
ptm <- proc.time()
...
for(tter in 1:length(vT)) for(nter in 1:length(vN)){
  ...
  for(mter in 1:iM) for(lter in 1:3) for(oter in 1:3){
    aRet1[lter,oter,tter,nter,mter] = RET[[mter]][[1]][oter,lter]
    aRet2[lter,oter,tter,nter,mter] = RET[[mter]][[2]][oter,lter]
  }
  cat(">")
  
}

save.image("RESULTS/EXPECV02_1.RData") #new
